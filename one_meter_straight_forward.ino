 ...
 * Modified by: Di Filippo Davide (Sept 10, 2022)
 * Enable and number of pair poles end wheels dimension has been changed 
 * working code on L293D 
 * TODO i'm searching for a way to have code working on arduino motor drive - problems with board hardware pwm control  on pin 2 and external interrupt
 * on the same pin for encoder count
 * Use two magnetic encoders on Fred's motor shafts (60:1
 * gearbox, 65mm wheels) to make him move in a straight line for
 * 1m.
 *
...
const int counts_per_rev = 1248;   // (5 pairs N-S) * (60:1 gearbox) * (2 falling/rising edges) = 384
...
void setup() {
  ...
  //drive(motor_power, motor_power); //test only
...
void loop() {
  // Do nothing
    ...
    /*if ( enc_l_prev < enc_l || enc_r_prev < enc_r){
      // Print out current number of ticks
      Serial.print(enc_l);
      Serial.print("\t");
